...
int main() {
    ...
    test<int>("Get at below lower bound", []() {
        int items[] = {1, 2, 3, 4, 5};
        DoublyLinkedList<int> l = DoublyLinkedList<int>::build(items, 5);

        try {
            l.get_at(-1);
        } catch (int err) {
            return create_pair(DoublyLinkedList<int>::INDEX_ERROR, err);
        }
    ...
    test<int>("Remove on empty", []() {
        ...
        try {
            ...
            return create_pair(DoublyLinkedList<int>::LIST_IS_EMPTY, err);
        }
        return create_pair(-1, -1);
    }, is_eq);
